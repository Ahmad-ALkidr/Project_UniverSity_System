...
class University():
    def __init__(self, window):
        ...
        self.buttonlogout = Button(self.frametop, text="Logout", command=self.logout, image=self.new_im, bd=10)
...
class Login:
    def __init__(self, window):
        ...
        self.EntryUser = Entry(self.frameEnter, width=23, font=('Tahoma', 12), bd=2)
        self.EntryUser.grid(row=0, column=1, pady=10)
        self.Entrypass = Entry(self.frameEnter, width=23, font=('Tahoma', 12), show='*', bd=2)
    ...
    def search(self):
        if self.ch1.get() == 1:
            ...
            mb.showerror('Error', "لم يتم تفعيل خيار نسيت كلمة المرور", parent=self.master)
