...
class Library:
    def __init__(self, bottomFrame):
        ...
        self.libraryFrame.grid(row=1, column=0, sticky='nsew', pady=5)
        self.img3 = Image.open(
            'image/png-transparent-subject-international-english-computer-icons-symbol-english-miscellaneous-blue-english-removebg-preview.png')
...
class library:
    def __init__(self):
        ...
        self.id_college = Entry(self.frameleft, fg='#4F4F4F', font=('tahoma', 12, 'bold'),
                                textvariable=self.id_college1, bd=2)
        ...
        self.add = Button(self.frameleft, command=self.add, image=self.new_im0, bg='#D0D3D4',
        ...
        self.Re = Button(self.frameright, command=self.registration, text="registration", bg='#D0D3D4',
                         activeforeground='white', activebackground='#750E21', font=('tahoma', 10, 'bold'),
                         cursor='plus', bd=15)
    ...
    def add(self):
        ...
        if (len(self.nameSubject.get()) == 0 or len(self.unitSubject.get()) == 0 or len(
            ...
            if self.nameSubject.get().isalpha():
                ...
                mb.showerror('Error', "الاسم المادة بياناته غير صحيحة", parent=self.master)
    ...
    def registration(self):
        ...
        self.add1 = Button(self.regL, command=self.add_registration, text="Add", bg='#1b9ea4', width=7, pady=10, bd=10)
        self.add1.place(x=50, y=200)
        self.Update1 = Button(self.regL, command=self.update1, text="Update", bg='#1b9ea4', width=7, pady=10, bd=10)
        self.Update1.place(x=125, y=200)
        self.Delete1 = Button(self.regL, command=self.delete1, text="Delete", bg='#1b9ea4', width=7, pady=10, bd=10)
        self.Delete1.place(x=50, y=270)
        self.Show1 = Button(self.regL, command=self.read1, text="Show", bg='#1b9ea4', width=7, pady=10, bd=10)
        self.Show1.place(x=125, y=270)
        self.rest = Button(self.regL, command=self.Reset1, text="Rest", bg='#1b9ea4', width=7, pady=10, bd=10)
            ...
            if self.id_St.get().isdigit():
                if self.id_Sub.get().isdigit():
                    if self.result.get().isdigit():
                        ...
                        mb.showerror('Error', "حقل رقم الطالب بياناته غير صحيحة", parent=self.master)
                else:
                    mb.showerror('Error', "حقل رقم المادة بياناته غير صحيحة", parent=self.master)
            else:
                mb.showerror('Error', "حقل النتيجة بياناته غير صحيحة", parent=self.master)
